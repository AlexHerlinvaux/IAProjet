...
class Game :
    ...
    def starting(self):
        ...
        if not self._p1.is_human :
            with open("output_keys.csv", "r") as csvfile_keys:
                with open("output_vals.csv", "r") as csvfile_vals:
...
"""
import time 
p1 = Player("p1","pass1",is_human=False)
p2 = Player("p2","pass2",is_human=False)
...
for i in range(0,10):
    while game.state != "Over":
        for p in range(0,2):
            if game.state != "Over":
                state = game.board, game.player(p).position, game.turn
                game.player(p).action(state)
